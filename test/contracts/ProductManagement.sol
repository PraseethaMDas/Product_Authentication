...
contract ProductManagement {
    ...
    function concatenateInfoAndHash(address a1, string memory s1, string memory s2, string memory s3, string memory s4, string memory s5,string memory s6) private returns (bytes32){
        ...
        string memory s_full = new string(b_a1.length + b_s1.length + b_s2.length + b_s3.length + b_s4.length + b_s5.length + b_s6.length);
    ...
    function buildPart(string memory serial_number, string memory part_type, string memory creation_date, string memory expiry_date, string memory product_price, string memory selling_date) public returns (bytes32){
        //Create hash for data and check if it exists. If it doesn't, create the part and return the ID to the user
        bytes32 part_hash = concatenateInfoAndHash(msg.sender, serial_number, part_type, creation_date, expiry_date, product_price,selling_date);
        
        require(parts[part_hash].manufacturer == address(0), "Part ID already used");

        Part memory new_part = Part(msg.sender, serial_number, part_type, creation_date, expiry_date, product_price, selling_date);
        parts[part_hash] = new_part;
        return part_hash;
    }

    function setSellDate(bytes32 p_hash, string memory sdate) public returns (bytes32){
        //Create hash for data and check if it exists. If it doesn't, create the part and return the ID to the user
        parts[p_hash].selling_date = sdate;
    }
